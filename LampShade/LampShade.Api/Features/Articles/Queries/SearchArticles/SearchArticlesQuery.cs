...
namespace LampShade.Api.Features.Articles.Queries.SearchArticles;
...
public class SearchArticlesQueryHandler : IRequestHandler<SearchArticlesQuery, List<ArticleViewModel>>
{
    ...
    public async Task<List<ArticleViewModel>> Handle(SearchArticlesQuery request, CancellationToken cancellationToken)
    {
        ...
        return await Task.FromResult(_articleApplication.Search(searchModel));
    }
}
